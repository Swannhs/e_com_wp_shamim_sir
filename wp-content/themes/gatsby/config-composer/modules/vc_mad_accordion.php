<?php
if ( !class_exists('gatsby_vc_accordion') ) {

	class gatsby_vc_accordion {
		...
		public function enqueue_admin_scripts() {
			...
			wp_enqueue_script( 'gatsby-vc-accordion-admin', get_template_directory_uri() . '/config-composer/assets/js/js_accordion_admin_enqueue.js', array( 'jquery' ), true );
			wp_enqueue_script( 'gatsby-vc-accordion-single', get_template_directory_uri() . '/config-composer/assets/js/js_accordion_single_element.js', array( 'jquery' ), true );
		}
		...
		function add_map() {
			...
			if ( function_exists('vc_map') ) {
				...
				vc_map( array(
					"name"    => esc_html__('Accordion', 'gatsby') ,
					...
					"description" => esc_html__("Collapsible content panels", "gatsby"),
					'js_view' => 'GatsbyVcBackendTtaAccordionView',
					'params' => array(
						array(
							'type' => 'textfield',
						...
						array(
							...
							'value' => array(
								...
								esc_html__( 'Secondary', 'gatsby' ) => 'gt-type-2',
							),
							'heading' => esc_html__( 'Style', 'gatsby' ),
							'description' => esc_html__( 'Select accordion display style.', 'gatsby' ),
						),
						array(
							'type' => 'checkbox',
							'heading' => esc_html__( 'Use toggle element?', 'gatsby' ),
							'param_name' => 'toggle',
							'description' => esc_html__( 'If checked, will be used toggle element.', 'gatsby' ),
									 ...
									 <div class="vc_tta-panel-heading">
										<h4 class="vc_tta-panel-title vc_tta-controls-icon-position-left">
										   <a href="javascript:;" aria-expanded="false" class="vc_tta-backend-add-control">
											   <span class="vc_tta-title-text">' . esc_html__( 'Add Section', 'gatsby' ) . '</span>
					...
					'default_content' => '[vc_mad_tta_section title="' . sprintf( '%s %d', __( 'Section', 'gatsby' ), 1 ) . '"][/vc_mad_tta_section][vc_mad_tta_section title="' . sprintf( '%s %d', __( 'Section', 'gatsby' ), 2 ) . '"][/vc_mad_tta_section]',
				));
				...
				vc_map( array(
					...
					'category' => esc_html__( 'Gatsby', 'gatsby' ),
					'js_view' => 'GatsbyVcBackendTtaSectionView',
					'params' => array(
						array(
							'type' => 'textfield',
							'param_name' => 'title',
	...
	if ( class_exists('WPBakeryShortCodesContainer') ) {
		...
		class WPBakeryShortCode_VC_mad_accordion extends WPBakeryShortCodesContainer {
			...
			public function getTtaContainerClasses() {
				...
				return implode( ' ', apply_filters( 'gatsby_vc_tta_mad_container_classes', array_filter( $classes ), $this->getAtts() ) );
			}

			public function getTtaGeneralClasses() {
				$classes = array();
				$classes[] = 'gt-accordion';
				$classes[] = $this->getTemplateVariable( 'style' );

				return implode( ' ', apply_filters( 'gatsby_vc_tta_mad_accordion_general_classes', array_filter( $classes ), $this->getAtts() ) );
			}
			...
			public function getParamTitle( $atts, $content ) {
				...
				return Gatsby_Vc_Config::getParamTitle(
					array(
						'title' => $atts['title'],
						'tag_title' => $atts['tag_title'],
