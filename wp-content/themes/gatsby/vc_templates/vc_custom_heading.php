 ...
 */
$source = $text = $link = $google_fonts = $font_container = $el_class = $css = $font_container_data = $with_border = $google_fonts_data = $animation = $css_animation = $animation_delay = $scroll_factor = '';
...
$border_class = '';
if ( $with_border ) {
	$border_class = 'gt-with-border-' . $font_container_data['values']['text_align'];
}
...
if ( apply_filters( 'vc_custom_heading_template_use_wrapper', false ) ) {
	$output .= '<div class="' . esc_attr( $css_class ) . ' '. $animation .' ' . esc_attr($border_class) .  '" >';
	$output .= '<' . $font_container_data['values']['tag'] . ' ' . $style . ' >';
	$output .= $text;
	$output .= '</' . $font_container_data['values']['tag'] . '>';
	$output .= '</div>';
} else {
	$output .= '<' . $font_container_data['values']['tag'] . ' ' . $style . ' '. $animation .' class="' . esc_attr( $css_class ) . ' ' . esc_attr($border_class) . '">';
	$output .= $text;
	$output .= '</' . $font_container_data['values']['tag'] . '>';
}
