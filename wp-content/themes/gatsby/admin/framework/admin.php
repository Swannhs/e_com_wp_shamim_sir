...
if ( !class_exists('Gatsby_Admin') ) {
	...
	class Gatsby_Admin {
		...
		public function enqueue_scripts() {
			wp_localize_script('jquery', 'gatsby_admin_global_vars', array(
				'installing_text' => esc_html__( 'Installing', 'gatsby' ),
				'import_theme_options_text' => esc_html__( 'Importing theme options', 'gatsby' ),
				'finished_text' => esc_html__( 'Finished! Please visit your site.', 'gatsby' ),
			));
		}
		...
		function export_sidebars() {
			$custom_sidebars = get_option('gatsby_sidebars');
		...
		public function export_metadata() {
			...
			$meta_settings = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}product_catmeta"), ARRAY_A);
	...
	require_once( get_template_directory() . '/admin/framework/theme-options.php' );
