(function ($, window) {
	...
	function gatsby_sidebar_generator () {
		var base = this;
			base.widgetArea = $('#widgets-right');
			base.widgetTemplate  = $('#gatsby-tmpl-add-widget');
	...
	gatsby_sidebar_generator.prototype = {
		createForm: function ()  {
            this.widgetLiquid.append(this.widgetTemplate.html());
            this.nonce = this.widgetLiquid.find('input[name="gatsby-custom-sidebar-nonce"]').val();
        },
		createRemoveButton: function () {
            this.widgetArea.find('.sidebar-gatsby-widget-custom').append('<span class="custom-sidebar-delete dashicons-trash"></span>');
        },
