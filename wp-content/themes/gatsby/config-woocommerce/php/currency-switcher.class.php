...
if (!class_exists('Gatsby_WC_Current_Switcher')) {

	class Gatsby_WC_Current_Switcher extends Gatsby_WooCommerce_Config {
		...
		function __construct() {
			...
			$this->settings = array(
				array( 'name' => esc_html__( 'Currency Codes', 'gatsby' ), 'type' => 'title', 'desc' => '', 'id' => 'wc_currency_code' ),
				array(
					'name' => esc_html__('Codes', 'gatsby'),
					'desc' 		=> '',
				...
				array( 'name' => esc_html__( 'Open Exchange Rate API', 'gatsby' ), 'type' => 'title', 'desc' => '', 'id' => 'product_enquiry' ),
				array(
					'name' => esc_html__('App Key', 'gatsby'),
					'desc' 		=> sprintf( wp_kses(__('(optional) If you have an <a href="%s">Open Exchange Rate API app ID</a>, enter it here.', 'gatsby'), array('a' => array('href' => array()))), 'https://openexchangerates.org/signup' ),
		...
		public function enqueue_currency_js() {
			...
			wp_enqueue_script( 'gatsby_money_js', $assets_js_url . 'money' . (WP_DEBUG ? '' : '.min') . '.js', array( 'jquery', 'gatsby-woocommerce-mod' ), '', true );
			...
			wp_localize_script( 'gatsby-woocommerce-mod', 'wc_currency_converter_params', array(
				...
				'i18n_oprice'      => esc_html__( 'Original price:', 'gatsby'),
	...
	new Gatsby_WC_Current_Switcher();
