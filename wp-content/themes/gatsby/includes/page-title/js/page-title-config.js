(function ($) {
	...
	$.gatsby_page_title = $.gatsby_page_title || {};

	/*	Init
	/* --------------------------------------------- */

	$.gatsby_page_title.init = function () {

		$('#gatsby_page_title_meta_box').on('click', '.add_image_available_media', function (e) {
			$.gatsby_page_title.add_available_media(e);
		}).on('click', '.remove_image_available_media', function (e) {
			$.gatsby_page_title.remove_image(e);
		}).find('select, input').styler();

		if ( $('.gatsby-page-title-color-picker').length ) {
			$('.gatsby-page-title-color-picker').wpColorPicker();
		}

		$('#gatsby_page_title_meta_box input:radio').on('change', function () {
		...
		//$('.gatsby-required-section').gatsby_page_title_required();
	...
	$.fn.gatsby_page_title_required = function () {
		return this.each(function(id, el) {
			$.gatsby_page_title.required(el);
		});
	}

	$.gatsby_page_title.required = function (el) {
		...
		return {
			...
			init: function () {
				this.data = {
					el: this.element,
					required : $('.gatsby-type-field-required', this.element).val().split(':')
				};
	...
	$.gatsby_page_title.add_available_media = function (e) {
		...
		var $element = $(e.target),
			...
			nonce = $('input[name=gatsby_page_title_meta_box_nonce]', $d_target).val(),
			data = {
				action: 'gatsby_page_title',
				type: type,
				gatsby_page_title_meta_box_nonce: nonce
			};
	...
	$.gatsby_page_title.remove_image = function (e) {
		e.preventDefault();
		...
		$img.attr('src', gatsby_page_title_vars.img);
		$hidden.val('');
