...
if ( ! class_exists('Gatsby_Envato_API') ) :
	...
	class Gatsby_Envato_API {
		...
		/**
		 * Main Gatsby_Envato_API Instance
		 *
		 * Ensures only one instance of this class exists in memory at any one time.
		 *
		 * @see Gatsby_Envato_API()
		 * @uses Gatsby_Envato_API::init_globals() Setup class globals.
		 * @uses Gatsby_Envato_API::init_actions() Setup hooks and actions.
		 *
		 * @since 1.0.0
		 * @static
		 * @return object The one true Gatsby_Envato_API.
		 * @codeCoverageIgnore
		 */
		 ...
		 *
		 * @see Gatsby_Envato_API::instance()
		 *
		 * @since 1.0.0
		 * @access private
		 * @codeCoverageIgnore
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'gatsby' ), '1.0.0' );
		}
		...
		private function init_globals() {
			// Envato API token.
			self::$token = Gatsby()->registration->get_token();
		}
		...
		public function request( $url, $args = array() ) {
			$defaults = array(
				'headers' => array(
					...
					'User-Agent' => 'WordPress - Gatsby ' . Gatsby()->get_theme_version(),
				),
				'timeout' => 20,
			);
			...
			if ( empty( self::$token ) ) {
				return new WP_Error( 'api_token_error', __( 'An API token is required.', 'gatsby' ) );
			}
			...
			if ( 200 !== $response_code && ! empty( $response_message ) ) {
				...
				return new WP_Error( $response_code, __( 'An unknown API error occurred.', 'gatsby' ) );
			} else {
				$return = json_decode( wp_remote_retrieve_body( $response ), true );
				if ( null === $return ) {
					return new WP_Error( 'api_error', __( 'An unknown API error occurred.', 'gatsby' ) );
				}
				return $return;
			}
		}
		...
		public function deferred_download( $id ) {
			...
			return add_query_arg( $args, esc_url( admin_url( 'admin.php?page=gatsby' ) ) );
		}
