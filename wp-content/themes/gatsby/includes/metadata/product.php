...
if (!class_exists('Gatsby_Metadata_Product')) {

	class Gatsby_Metadata_Product {
		...
		function add_enqueue_styles() {
			...
			wp_enqueue_style( 'gatsby_metadata-mod', $css_file );
		}
		...
		function show_tax_add_meta_boxes($meta_boxes) {
			...
			foreach ($meta_boxes as $meta_box) {
				gatsby_show_tax_add_meta_html($meta_box);
			}
		}
		...
		function show_tax_edit_meta_boxes($term, $taxonomy, $meta_boxes) {
			...
			foreach ($meta_boxes as $meta_box) {
				gatsby_show_tax_edit_meta_html($term, $taxonomy, $meta_box);
			}
		}
		...
		function product_get_postdata() {
			global $product_cat_meta_array;
			$product_cat_meta_array = gatsby_cat_meta_view();
		}
		...
		function save_product_cat($term_id, $tt_id, $taxonomy) {
			...
			return gatsby_save_taxdata( $term_id, $tt_id, $taxonomy, $product_cat_meta_array );
		}

	}

	new Gatsby_Metadata_Product();
